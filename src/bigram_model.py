...
def bigrams_count_to_probabilities(
    ...
    epsilon = 1e-10
    row_sums = bigram_counts.sum(dim=1, keepdim=True) + epsilon
    bigram_probabilities = bigram_counts / row_sums
...
def calculate_neg_mean_log_likelihood(
    ...
    for word in words:
        word_log_likelihood = calculate_log_likelihood(
...
def calculate_log_likelihood(
    ...
    for i in range(len(processed_word) - 1):
        ...
        bigram_prob = bigram_probabilities[current_index, next_index]
        
        log_likelihood += torch.log(bigram_prob + epsilon)
